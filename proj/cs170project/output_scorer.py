...
def score_output(input_folder, output_file):
    ...
    '''
    graph = nx.read_gml(os.path.join(input_folder, "graph.gml"))
    parameters = open(os.path.join(input_folder, "parameters.txt"))
...
if __name__ == '__main__':
    ...
    for i in range(1000, 1100):
        try:
            number = str(i)
            HOME = os.getcwd()
            input_folder = os.path.join(HOME, "inputs", size, number)
            output_folder = os.path.join(HOME, "outputs", size, number + ".out")
            
            score, msg = score_output(input_folder, output_folder)
            if score > -1:
                real_score += score
                total_score += 1
            print(size, "FILE", i, msg)
        except FileNotFoundError:
            total -= 1
            continue
    print("TOTAL SCORE IS", total_score, "out of", total, ":", str(total_score/total*100)+"%")
    print("REAL SCORE IS", str(real_score/total)) 